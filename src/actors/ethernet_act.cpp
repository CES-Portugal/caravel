#include "../../include/actors/ethernet_act.h"
...
int setup_tcp_socket(){
    ...
    //Create socket
    sokt = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    if (sokt < 0) {
        perror("socket PF_INET failed");
        return 1;
    }

    //Specify eth0 device
    strcpy(ifr.ifr_name, "wlan0");
    ret = ioctl(sokt, SIOCGIFADDR, &ifr);
    if (ret < 0) {
        perror("ioctl failed");
        return 1;
    }

    /* int status = fcntl(sokt, F_SETFL, fcntl(sokt, F_GETFL, 0) | O_NONBLOCK);
    if (status == -1){
        perror("calling fcntl");
        return 1;
    } */

    if (setsockopt(sokt, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr)) < 0)
    {
    ...
    ret = bind(sokt, (struct sockaddr *)&addr, sizeof(addr));
    if (ret < 0) {
        perror("bind failed");
        return 1;
    } */
...
void ethernet_act(event_based_actor* self){
    
    //Test ethernet
    signal(SIGINT, sighandler);
	umask(0);
 
	log_file=fopen("./ethernet/log.txt","a");
	if(!log_file)
	{
		perror("unable to open log.txt\n");
		return;
	}

    summary_file=fopen("./ethernet/summary.txt","a");
    if(!summary_file)
	{
		perror("unable to open summary.txt\n");
		return;
	}

    mac_list_file=fopen("./ethernet/mac_list.txt","a+t");
    if(!mac_list_file)
	{
		perror("unable to open mac_list.txt\n");
		return;
	}
    ...
    while (v < 100)
    {
        ...
        data_process(buffer, nbytes);
		fflush(log_file);
		// fflush(summary_file);
		fflush(mac_list_file);
        
        //cout << "Received message! "<< nbytes << endl << endl;
